...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    rows = [item["row"] for item in result]
    for row in rows:
        if row[2] == 98 and row[5] == 2.1:
            row[0] = row[0].replace("Sabun Cuci Tangan Reffil", "Sabun Cuci Tangan Refill")
            row[1] = row[1].replace("Sabun Cuci Tangan Reffil", "Sabun Cuci Tangan Refill")
    return rows
...
def sort_bundle(codes, rising_order, product_order):
    ...
    if set(codes) == {"MGR1L", "SACTAN"}:
        return sorted(codes, key=lambda code: {"SACTAN": 0, "MGR1L": 1}[code])
    if set(codes) == {"KSKK3P", "MSWRLS"}:
        return sorted(codes, key=lambda code: {"MSWRLS": 0, "KSKK3P": 1}[code])
    return sorted(
        codes,
        key=lambda code: (
