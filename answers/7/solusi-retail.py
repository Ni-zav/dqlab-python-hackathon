...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    result = []
    for rule in rules:
        antecedent_has_rising = bool(rule["antecedents"] & rising_codes)
        consequent_has_rising = bool(rule["consequents"] & rising_codes)
        if antecedent_has_rising == consequent_has_rising:
            continue
