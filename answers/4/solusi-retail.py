...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    result = []
    for rule in rules:
        antecedents = sort_bundle(rule["antecedents"], rising_order, product_names)
        consequents = sort_bundle(rule["consequents"], rising_order, product_names)
        result.append(
            {
                "row": [
...
def sort_bundle(codes, rising_order, product_names):
    return sorted(
        codes,
        key=lambda code: (
            0 if code in rising_order else 1,
            rising_order.get(code, product_names[code]),
        ),
    )
