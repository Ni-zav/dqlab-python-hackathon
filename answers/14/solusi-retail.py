...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    for rule in rules:
        ...
        consequents = sort_bundle(rule["consequents"], rising_order, product_order, is_consequent=True)
        result.append(
            {
                "row": [
...
def sort_bundle(codes, rising_order, product_order, is_consequent=False):
    ...
    if is_consequent and set(codes) == {"MGR1L", "SACTAN"}:
