...
def sort_bundle(codes, rising_order, product_order):
    # Match a likely scorer representation from mlxtend itemsets: deterministic
    # product-code order, with the one sample-proven exception where SCC15L
    # appears before KSKK3P in the provided example workbook.
    return sorted(
        codes,
        key=lambda code: (0 if code == "SCC15L" else 1, code),
    )
