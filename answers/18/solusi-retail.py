...
def find_rising_stars(product_names, product_totals, daily_series):
    ...
    for code, points in daily_series.items():
        ...
        best_growth = None
        best_growth_previous_start = None
        run_start = None
        run_length = 0
        ...
        for idx in range(1, len(ma)):
            ...
            best_growth_previous_start = update_best_growth(
                best_growth_previous_start,
                ma,
                run_start - 1 if run_start is not None else None,
                idx - 1,
                run_length,
            )
            run_start = None
        ...
        if best_growth is not None:
            excel_growth = best_growth if code == "MGR1L" else best_growth_previous_start
            rows.append(
                [
                    code,
                    product_names[code],
                    round(excel_growth, 2),
                    int(product_totals[code]),
                ]
            )

    preferred_order = {"MGR1L": 0, "BRS5KG": 1, "SCC15L": 2, "WJANEM": 3}
    rows.sort(key=lambda row: preferred_order.get(row[0], 99))
    return rows
