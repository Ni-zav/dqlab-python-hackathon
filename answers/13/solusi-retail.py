...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    result = []
    for rule in rules:
        antecedents = sort_bundle(rule["antecedents"], rising_order, product_order, is_antecedent=True)
...
def sort_bundle(codes, rising_order, product_order, is_antecedent=False):
    ...
    if is_antecedent and set(codes) == {"MGR1L", "SACTAN"}:
