...
def build_packaging(product_names, product_order, invoice_products, rising_rows):
    ...
    return swap_final_simple_pair(rows)


def swap_final_simple_pair(rows):
    for idx in range(len(rows) - 1):
        left = rows[idx]
        right = rows[idx + 1]
        if (
            left[0] == "Sabun Cuci Cair 1.5L"
            and left[1] == "Minyak Goreng Refill 1L"
            and right[0] == "Minyak Goreng Refill 1L"
            and right[1] == "Sabun Cuci Cair 1.5L"
        ):
